...
class AdminCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.admin_guilds = [551837071703146506]
    
    @slash_command(name="update", description="Pulls the latest version from GitHub (owner-only).", guild_ids=self.admin_guilds)
    ...
    @slash_command(name="load", description="Loads a previously unloaded module (owner-only).", guild_ids=self.admin_guilds)
    @permissions.is_owner()
    ...
    @slash_command(name="unload", description="Unloads a previously loaded module (owner-only).", guild_ids=self.admin_guilds)
    @permissions.is_owner()
    ...
    @slash_command(name="reload", description="Reloads a previously loaded module (owner-only).", guild_ids=self.admin_guilds)
    @permissions.is_owner()
    ...
    @slash_command(name="servers", description="Displays a list of all servers the bot is in (owner-only).", guild_ids=self.admin_guilds)
    @permissions.is_owner()
    ...
    @slash_command(name="error", description="Forcibly causes the bot to error, for debugging purposes (owner-only).", guild_ids=self.admin_guilds)
    @permissions.is_owner()
